...
bomb_launcher_projectile::bomb_launcher_projectile()
{
	...
	is_active_ = false;
	fire_rate_ = 10;
	//fire_rate_counter_bomb = fire_rate_;
}
...
void bomb_launcher_projectile::visual_adjustments()
{
	...
	int width = texture_bomb_launcher_projectile_.getSize().x / nb_projectile_frames;
	int height = texture_bomb_launcher_projectile_.getSize().y / nb_projectile_anims;
...
{
	//fire_rate_counter_bomb++;
}

void bomb_launcher_projectile::shoot(Vector2f position, Vector2f direction)
{
	//if (fire_rate_counter_bomb >= fire_rate_)
	//{
		is_active_ = true;
		direction_ = direction;
		setPosition(position);
	//	fire_rate_counter_bomb = 0;
	//}
}